...
/*
static sem_t HumbleLock;

#define LOCK_INIT()     sem_init(&HumbleLock, 0, 1)
#define LOCK_WAIT()     sem_wait(&HumbleLock)
#define LOCK_TRY_WAIT() (sem_trywait(&HumbleLock) == 0)
#define LOCK_RELEASE()  sem_post(&HumbleLock)
#define LOCK_DESTROY()  sem_destroy(&HumbleLock)
...
int libwebrtc_create_offer( struct libwebrtc_connection* c ) {
    ...
    ctx->callback(ctx, c, NULL, LWRTC_CALLBACK_LOCAL_DESCRIPTION, user_data, offer, strlen(offer));
